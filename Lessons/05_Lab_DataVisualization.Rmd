...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #paste allows you to concatenate strings, [3] would give you subscript
print(Plot_text)
...
plot1 <-
  ggplot(filter(PeterPaul.chem.nutrients, depth == 0), 
                aes(x = daynum, y = temperature_C)) + # can also put color here, could make more sense bc we already have aes()
  geom_point(aes(color = year4), alpha=0.5) + #alpha changes transparency
...
plot2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C)) + #if you want different dataset in each geom, need to specify in EACH geom. if same dataset for each geom, can specify in ggplot
...
Exercise: Plot relationships between air quality measurements (EPAair)
...
plot3 <- ggplot(EPAair, aes(x=PM2.5, y=Ozone)) + #can put either variable on x or y
  geom_point(aes(color = mean_lat), alpha = 0.5) +
  geom_smooth(method = lm)
print(plot3)

plot3_other <- ggplot(EPAair, aes(x=PM2.5, y=Ozone, color = mean_lat)) + #should group color in aes()
