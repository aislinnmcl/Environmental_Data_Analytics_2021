...
> My Questions: Why does `str(NTL.phys.data) label Factor instead of chr?
Why don't we filter by row number (don't understand the q)?

************************************

> Add notes here: Could affect algal growth which would affect water quality.
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma same as ampersand
...
NTL.phys.data.1990thru1999 <- filter(NTL.phys.data, year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Filter only works on column values?
...
NTL.phys.data.temp.ascending <- arrange(NTL.phys.data, desc(temperature_C))
...
> You don't want R script to get too long. Plus it will make multiple analyses cleaner.
