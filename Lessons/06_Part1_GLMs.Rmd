...
Standard error is measure of the scatter of points around a line.
Want it to be as small as possible.
Correlation coefficient measures strength and direction of relationship between 2 variables (range -1 to +1).
R2 measures the percentage of variability in the values of y that are "explained" by the x variable (range 0 to 1).

## Hypothesis Testing

If p-value ≤ alpha, reject the null hypothesis.
If p-value > alpha, do not reject the null hypothesis.
x-axis has independent variable, y-axis has dependent variable.
...
#depth explains ~30% of irradiance
#p-value smaller than confidence level (.05) tells us regression is meaningful
...
> We see there is a significant negative correlation between irradiance and depth (lower light levels at greater depths), and that this model explains about 31 % of the total variance in irradiance. Let's visualize this relationship and the model itself. 
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) #divides screen into 4 cells, sets size of windows
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) +
  geom_point() 
print(irradiancebydepth) #we don't have a linear relationship between variables
```
...
PeterPaul.chem.nutrients <- filter(PeterPaul.chem.nutrients, 
                                   irradianceWater != 0 & irradianceWater < 5000) #eliminate zeros to take log, also eliminate outlier
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) #depth is independent variable
summary(irradiance.regression2) #after log transformation, R2 is ~73%, x explains y better
...
plot(TPregression)
par(mfrow = c(1,1))
#for plots we want red line to be flat
...
corrplot.mixed(PeterPaulCorr, upper = "ellipse") # print upper diagonal only because plot is symmetric, allows us to also show value associated w ellipse
```
...
summary(TPmodel) #hoping for a better R2 but we actually got a worse R2
