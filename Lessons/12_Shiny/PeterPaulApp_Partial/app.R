...
nutrient_data$sampledate <- as.Date(nutrient_data$sampledate, "%Y-%m-%d")
nutrient_data <-
  nutrient_data %>%
  filter(depth_id >= 0) %>%
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput(inputId = "x",
                  ...
                  max = as.Date("2016-08-31"),
                  value = c(as.Date("1991-01-01"), as.Date("1996-12-31")))),
...
server <- function(input, output) {
    ...
    # Create a table that generates data for each point selected on the graph  
       output$mytable <- renderTable({
         brush_out <- brushedPoints(filtered_nutrient_data(), input$scatterplot_brush)# dataset, input
       }) 
