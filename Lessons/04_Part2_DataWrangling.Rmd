...
*******************************
>My Questions: Why does summary display the other lakes even though you filtered for Peter and Paul and assigned to an object? Why would you use a pipe if only doing one operation e.g. select for LitterTrap? `summarise()` has grouped output by 'lakename'. You can override using the `.groups` argument...
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #assign dataframe
  mutate(month = month(sampledate)) %>% #add month column
  select(lakeid:daynum, month, sampledate:comments) %>% #move month column next to sampledate
  drop_na(depth) #remove NA depths
...
# Spread nutrient data into separate columns using pivot_wider, opposite of pivot_longer transformation
# Smaller than original dataset by 2 rows, probably because 2 rows had NA for all nutrient concentration values
