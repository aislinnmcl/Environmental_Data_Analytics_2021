...
vector3
vector4 <- c(1, "one") # defaults to broadest vector category
vector4

vector1[3] # 

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5, ncol = 4) # sequence of values from 1-20, 5 rows, 4 columns
matrix1
...
matrix1[4, ] # returns row 4, all columns
matrix1[ , 3] # returns column 3, all rows
matrix1[c(12, 14)] # gets values 12 and 14
matrix1[c(12:14)] # gets values 12 through 14
matrix1[2:4, 1:3] # rows 2 through 4, columns 1 through 3
...
list1[[2]] #one bracket returns one item from the list as list; two brackets returns numeric
#class(list[[2]])
#class(list[2])
...
dataframe1 <- data.frame(d,e,f) #because vectors are same length, they can be columns in df
...
> Answer: Multiple dimensions show up in Data tab, dfs and lists have dropdown. Vectors are mostly in values tab.

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: Tab allows you to choose a column
...
high_temp_F <- c(55, 54, 55, 54, 59, 52, 43, 36, 34, 38)
low_temp_F <- c(32, 32, 28, 39, 40, 30, 29, 22, 21, 23)
...
convert_C <- function(x) {
    (x-32)*(5/9)
}

high_temp_C <- convert_C(high_temp_F)
low_temp_C <- convert_C(low_temp_F)
...
temps_df <- data.frame(high_temp_F, low_temp_F, high_temp_C, low_temp_C)
names(temps_df) <- c("Highs F", "Lows F", "Highs C", "Lows C")
