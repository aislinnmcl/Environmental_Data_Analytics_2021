...
class(USGS.flow.data$datetime) # dates always brought in as character comment

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y-%m-%d") # tells computer this is a date and this is how its structured
...
# ggplot(dataframe, aesthetics)
# can do ggplot(na.omit) to eliminate any rows with no data
...
# default bins = 30, generally use 5-20 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# changing scale of x axis, lets us zoom in
...
# this is looking at the distribution of data
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# distribution of mean gage height values, thick black line shows median, box shows quantiles
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# slicing data with group and cut_width
# cut_width shows distribution of values in gage height mean
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# width is how many data points are in that tier
ggplot(USGS.flow.data) +
