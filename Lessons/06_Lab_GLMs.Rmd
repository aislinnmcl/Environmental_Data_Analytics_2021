...
EPAair <- rename(EPAair, Year = year)
...
summary(EPAair$Ozone) #mean is lower than 50, but could be thrown off by a lot of things which is why we still need t-test
EPAair.subsample <- sample_n(EPAair, 5000) #Shapiro test can only run on <= 5000 records

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) #p-value is probability that null hypothesis is true. null value is that data are normal. this test shows likelihood that our data are normal is small
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) #if data were normal, your data would lie along the line

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample #p-value is way less than .05, so probability that mean is >=50 is very small (probability that null hypothesis is true)
...
> On average, ozone measured in NC is less than 50 ppm over the time period of... (t-test, df = 6829, p-value < 0.001).
...
ggplot(data = EPAair, aes(x = Ozone, y = PM2.5)) +
  geom_point() +
