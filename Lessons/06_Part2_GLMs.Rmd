...
#check all value of plotID, some sites have been sampled more than others
...
#results: p-value < .05, reject null i.e. variances are not equal
...
# Format ANOVA as lm, main difference is summary output - lm has one row for each level of categorical variable. mean estimates are with respect to intercept
...
> Need to reorder sites (i.e. a, ab, abc) so you can see changes in the mean.
...
Litter.groups
#output in groups column tells you which groups can be compared (e.g. a cannot be compared with anything else, there is only one a group)
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  geom_boxplot() +
  labs(x = "NLCD Class", y = "Dry Mass", color = "Functional Group") +
    stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5,
               label = c("d", "d", "d", "d", "d", "a", "d", "d",
                         "d", "d", "d", "d", "d", "b", "d", "d", 
                         "d", "cd", "d", "d", "d", "bc", "d", "d")) #don't think this is right... ask in class
