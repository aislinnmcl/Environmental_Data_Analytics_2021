 ...
 * Spatial data structures: geometry, sfg, sfc, sf objects
      - sf = spatial features
      - sfc = spatial features column, just the geometry column
      - sfg = spatial feature geometry, just a single row of geometry (i.e. coordinates plus info on projection)
      - geometry = just set of coordinates
...
huc8_sf <-
  st_read("./Data/Spatial/NCHUC8.shp")
...
huc8_UpNeuse_sf <-
  st_read("./Data/Spatial/NCHUC8.shp") %>%
  dplyr::filter(SUBBASIN == "Upper Neuse")

#Create a map

mapview(huc8_sf, col.regions = "orange") +
  mapview(huc8_UpNeuse_sf, col.regions = "purple")
...
huc8_wind <-
  read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv",
           colClasses = c("HUC_12" = "factor")) %>%
  mutate(HUC8 = substr(HUC_12, 1, 8)) %>%
  group_by(HUC8) %>%
  dplyr::summarise(kWhkm2day = mean(kWhkm2day))
...
# ggplot looks good, not sure why mapview isn't working
ggplot(huc2_sf) +
  geom_sf()
...
mapview(the_hucs) +
  mapview(the_triangle, alpha.regions = 0, color = "red")
