...
# renaming colums so subsequent code works
EPAair <-
  EPAair %>%
  rename(Month = month, 
         Year = year)
...
# won't change anything, default theme is gray
...
# use to set theme for all subsequent plots generated
theme_set(mytheme)
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #%b is abbrev for month name, first three letters
  ylab(expression("O"[3]* " AQI Value")) + #changes y-axis label, subscript for "3"
...
NvsP2 <-
  ...
  scale_shape_manual(values = c(15, 17)) + #15 and 17 corrspond to specific shapes in scale_shape_manual
...
#orders by Endpoint count, increasing
Neonics$Endpoint <- with(Neonics, reorder(Endpoint, Endpoint, FUN=length))

ggplot(Neonics) +
  geom_bar(aes(x = Endpoint), fill = "purple") + 
  scale_x_discrete(limits = rev(levels(Neonics$Endpoint))) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
  
# no need to color bars differently, they have labels
