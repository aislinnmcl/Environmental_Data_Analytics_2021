...
#not right
#NTL.phys.data.1990thru1999 <- 
#  filter(NTL.phys.data, year4 %in% c(1990:1999)) %>%
# filter(NTL.phys.data, lakename == "Tuesday Lake")

NTL.phys.data.1990thru1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Not reproducible. If your dataset changes the row numbers aren't assigned to the same thing.
...
Ex4 <-
  Ex3 %>%
  filter(depth == 0) %>%
  summarise(mean_surface_temp_C = mean(temperature_C))
