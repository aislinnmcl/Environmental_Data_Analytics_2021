...
phos_plot <- ggplot(chemnutrients_PeterPaul_processed, aes(x = tp_ug, y = po4, color = lakename)) +
  geom_point() +
  geom_abline(slope = 0.2162, intercept = -0.5894) + #using geom_smooth(method = lm) gives 2 lines, but want one
  xlim(0, 150) +
  ylim(0, 50) +
...
box_temp <- ggplot(chemnutrients_PeterPaul_processed, aes(x = month, y = temperature_C, color = lakename)) +
  geom_boxplot() + 
  labs(color = "Lake Name") +
  xlab("Month") +
  ylab("Temperature (°C)")
...
box_tp <- ggplot(chemnutrients_PeterPaul_processed, aes(x = month, y = tp_ug, color = lakename)) +
  geom_boxplot() + 
  labs(color = "Lake Name") +
  xlab("Month") +
...
box_tn <- ggplot(chemnutrients_PeterPaul_processed, aes(x = month, y = tn_ug, color = lakename)) +
  geom_boxplot() + 
  labs(color = "Lake Name") +
  xlab("Month") +
...
needles_plot <-
  ...
  xlab("Date") +
  ylab("Dry Mass") +
  scale_color_discrete("NLCD Class")
print(needles_plot)
